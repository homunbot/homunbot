...
| CHH-3 | **Slack → Socket Mode** | 1 settimana | Da polling 3s a push reale. Rate limit handling (429). Elimina latenza 3-6s |
...
| PRO-1 | **Proactive messaging Discord** | 2 giorni | `default_channel_id` già in config, usarlo per send proattivi |
| PRO-2 | **Proactive messaging Slack** | 2 giorni | `chat.postMessage` su channel configurato |
